...
const categories = ['All', 'Logistics Technology', 'Startup Updates', 'Freight Industry', 'Ethiopian Economy'];
...
export default function BlogPage() {
  ...
  useEffect(() => { filterPosts(); }, [selectedCategory, searchQuery, posts]); // eslint-disable-line
      ...
      <div className="pt-20 min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-12 animate-pulse">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="bg-white rounded-2xl h-96"></div>)}
          </div>
        </div>
      </div>
    );
    ...
    <div className="pt-20 min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-xl text-gray-600">Insights and updates from Ethiochinet and the logistics industry</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="max-w-4xl mx-auto mb-12">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {categories.map((category) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-colors ${selectedCategory === category ? 'bg-teal-600 text-white' : 'bg-white text-gray-700 hover:bg-teal-50 hover:text-teal-600'}`}
              >
                {category}
              </button>
            ))}
                ...
                <Link href={`/blog/${post.slug}`}>
                  ...
                  <div className="p-6">
                    <div className="flex items-center space-x-2 mb-3">
                      <span className="text-xs font-semibold text-teal-600 bg-teal-50 px-2 py-1 rounded">{post.category}</span>
                      <span className="text-xs text-gray-500">{format(post.publishedAt?.toDate(), 'MMM dd, yyyy')}</span>
                    </div>
